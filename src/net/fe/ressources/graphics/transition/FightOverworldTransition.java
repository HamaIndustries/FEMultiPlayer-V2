...
public class FightOverworldTransition extends Transition {
	...
	public FightOverworldTransition(ClientOverworldStage to, Unit a, Unit b, Runnable callback) {
		...
		if (range == 1) {
			x = new float[] { FightStage.CENTRAL_AXIS - 56, FightStage.CENTRAL_AXIS, FightStage.CENTRAL_AXIS,
			        FightStage.CENTRAL_AXIS - 77, FightStage.CENTRAL_AXIS, FightStage.CENTRAL_AXIS + 56,
			        FightStage.CENTRAL_AXIS + 77, FightStage.CENTRAL_AXIS };
		} else {
			x = new float[] { FightStage.CENTRAL_AXIS - 76, FightStage.CENTRAL_AXIS - 21, FightStage.CENTRAL_AXIS - 29,
			        FightStage.CENTRAL_AXIS - 105, FightStage.CENTRAL_AXIS + 21, FightStage.CENTRAL_AXIS + 76,
			        FightStage.CENTRAL_AXIS + 105, FightStage.CENTRAL_AXIS + 29 };
		}
		y = new float[] { FightStage.FLOOR - 16, FightStage.FLOOR - 16, FightStage.FLOOR + 8, FightStage.FLOOR + 8,
		        FightStage.FLOOR - 16, FightStage.FLOOR - 16, FightStage.FLOOR + 8, FightStage.FLOOR + 8, };
		dx = new float[8];
		dy = new float[8];
		for (int i = 0; i < 8; i++) {
			dx[i] = (destX[i] - x[i]) * 3f;
