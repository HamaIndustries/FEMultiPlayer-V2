package net.fe.ressources.sound;
...
public class SoundTrack {
	...
	public static void loop(String name) {
		...
		try {
			...
			b = AudioLoader.getAudio("WAV", ResourceLoader.getResourceAsStream("res/music/" + setting + ".wav"));
			b.playAsMusic(1.0f, FEResources.getAudioVolume(), true);
		} catch (Exception e) {
			e.printStackTrace();
			System.err.println("Warn: Bad sound configuration: " + name);
			try {
				Audio b = AudioLoader.getAudio("WAV", ResourceLoader.getResourceAsStream("res/music/" + name + ".wav"));
				b.playAsMusic(1.0f, FEResources.getAudioVolume(), true);
			} catch (Exception f) {
			}
		...
		try {
			...
			if (jarFile.isFile()) { // Run with JAR file
				...
				while (entries.hasMoreElements()) {
					...
					if (name.startsWith(musPath + "/") && name.indexOf(".wav") > 0) { // filter
					                                                                  // according
					                                                                  // to
					                                                                  // the
					                                                                  // path
	...
	public static void restart() {
		if (!enabled)
			return;
		try {
			Audio a = AudioLoader.getStreamingAudio("WAV", ResourceLoader.getResource("res/music/" + current + ".wav"));
