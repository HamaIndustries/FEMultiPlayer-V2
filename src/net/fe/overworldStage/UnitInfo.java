...
public class UnitInfo extends Entity implements DoNotDestroy {
	...
	public void render() {
		...
		Renderer.drawRectangle(x + 2, y + 2, x + 318, y + 78, renderDepth, NEUTRAL);
		...
		Renderer.drawString("default_med", u.name + "   Lv" + u.getLevel(), x + 92, y + 4, renderDepth);
		...
		Renderer.render(mugshot, 0, 0, 1, 1, x + 46 - mugshot.getImageWidth() / 2, y + 76 - mugshot.getImageHeight(),
		...
		for (String stat : Arrays.asList("Str", "Mag", "Skl", "Spd")) {
			...
			Renderer.drawString("default_med", statN, x + 122 - (width - 1) / 2, y + y0, renderDepth);
			y0 += 14;
		}
		...
		for (String stat : Arrays.asList("Lck", "Def", "Res", "Mov")) {
			...
			int statN = u.getBase(stat);
			width = FEResources.getBitmapFont("default_med").getStringWidth(statN + "");
			Renderer.drawString("default_med", statN, x + 182 - (width - 1) / 2, y + y0, renderDepth);
			y0 += 14;
		}

		// Inventory
		Renderer.drawRectangle(x + 208, y + 20, x + 318, y + 78, renderDepth, NEUTRAL.darker(0.5f));
		y0 = 22;
		for (int i = 0; i < 4; i++) {
			if (items[i] != null) {
				items[i].render();
		...
		if (rescued != null) {
			// Main box
			Renderer.drawRectangle(x + 320, y, x + 110 + 320, y + 80, renderDepth, BORDER_DARK);
			Renderer.drawRectangle(x + 321, y + 1, x + 109 + 320, y + 79, renderDepth, BORDER_LIGHT);
			Renderer.drawRectangle(x + 322, y + 2, x + 108 + 320, y + 78, renderDepth, NEUTRAL);
			// Terrain name ribbon
			Renderer.drawRectangle(x + 323, y + 3, x + 107 + 320, y + 77, renderDepth, NEUTRAL.darker(0.5f));
			BitmapFont def = FEResources.getBitmapFont("default_med");
			width = def.getStringWidth("Rescued");
			Renderer.drawString("default_med", "RESCUED", x + 320 + 55 - width / 2, y + 10, renderDepth);
			// Separator
			Renderer.render(dragons, 0, 0, 1, 1, x + 37 + 320, y + 32, x + 75 + 320, y + 41, renderDepth);

			// Info
			Renderer.drawString("default_med", rescued.name + "  Lv" + rescued.getLevel(), x + 325, y + 47,
			        renderDepth);
			Renderer.drawString("default_med", "HP " + rescued.getHp() + "/" + rescued.getStats().maxHp, x + 325,
			        y + 63, renderDepth);
		}
	}
