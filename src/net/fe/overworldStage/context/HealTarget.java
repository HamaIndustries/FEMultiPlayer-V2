...
public class HealTarget extends SelectTargetContext {
	 ...
	 * @param u the u
	 */
	public HealTarget(ClientOverworldStage stage, OverworldContext context, Zone z, Unit u) {
		super(stage, context, z, u, true);
		selector = new StaffSelector(ClientOverworldStage.RIGHT_AXIS - 45, 75, new ArrayList<Weapon>());
	}
	...
	public void updateCursor() {
		super.updateCursor();
		selector.setStaves(unit.equippableStaves(Grid.getDistance(unit, getCurrentTarget())));
		selector.x = getCurrentTarget().x + 8 - 45 - stage.camX;
		if (selector.x < 0)
			selector.x = 0;
		if (selector.x + 90 > 384)
			selector.x = 384 - 90;
...
class StaffSelector extends Entity {
	...
	public void render() {
		Renderer.drawRectangle(x, y, x + 90, y + 20, renderDepth, FightStage.BORDER_DARK);
		Renderer.drawRectangle(x + 1, y + 1, x + 89, y + 19, renderDepth, FightStage.BORDER_LIGHT);
		Renderer.drawRectangle(x + 2, y + 2, x + 88, y + 18, renderDepth, FightStage.NEUTRAL);
