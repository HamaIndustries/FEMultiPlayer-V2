...
public class ClientOverworldStage extends OverworldStage {
	...
	/**
	 * The number of currently running messages. Should be used as a semaphore
	 * to prevent messages from running in parallel
	 */
	...
	public ClientOverworldStage(Session s) {
		...
		camMaxY = Math.max(0, grid.height * 16 - 240);
		{
			...
			Node[] n = units.stream().filter((Unit u) -> u.getHp() > 0)
			        .map((Unit u) -> new Node(u.getXCoord(), u.getYCoord())).toArray(Node[]::new);
			if (n.length > 0) {
	...
	public void removeExtraneousEntities(Entity... keep) {
		...
		for (Entity e : entities) {
			if (!(e instanceof DoNotDestroy || keeps.contains(e))) {
				removeEntity(e);
			}
		}
	}
	...
	public void includeInView(Node... coords) {
		...
		for (Node n : coords) {
			if (n.x < camX / 16 || n.x > camX / 16 + 23 || n.y < camY / 16 || n.y > camY / 16 + 15)
				allVisible = false;
		}
		if (allVisible)
			return;
	 ...
	 *
	 * @see #hasControl
	 * @param c true if this should respond to keyboard and mouse events, else
	 *        false
	 */
	 ...
	 *
	 * @see #setControl
	 * @return true, if this is responding to keyboard and mouse events, else
	 *         false
	 */
	...
	public void end() {
		...
		selectedUnit = null;
		// do the turn transition stuff now to prevent fast-fingering a second
		// end turn
	...
	protected void doStartTurn(int playerID) {
		...
		if (FEMultiplayer.getLocalPlayer().getID() == getCurrentPlayer().getID()) {
			...
			if (FEResources.getAutoCursor().applyAtStartOfLocalTurn) {
				List<Unit> units = FEMultiplayer.getLocalPlayer().getParty().getUnits();
				Node[] n = units.stream().filter((Unit u) -> u.getHp() > 0 && !u.isRescued())
				        .map((Unit u) -> new Node(u.getXCoord(), u.getYCoord())).toArray(Node[]::new);
				if (n.length > 0) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.fe.overworldStage.OverworldStage#beforeTriggerHook(net.fe.
	 * overworldStage.TerrainTrigger, int, int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.fe.overworldStage.OverworldStage#processCommands(net.fe.network.
	...
	public void processCommands(final CommandMessage message) {
		...
		for (int i = message.commands.length - 1; i >= 0; i--) {
			...
			// only execute if originated from another server or if the command
			// has info added from the server
