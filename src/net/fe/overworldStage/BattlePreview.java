...
public class BattlePreview extends Entity {
	...
	public BattlePreview(float x, float y, Unit a, Unit d, int range) {
		...
		rightArrow.addAnimation("default", FEResources.getTexture("gui_selectArrow"), 8, 8, 6, 6, 0.1f);
		leftArrow = new Sprite();
		leftArrow.addAnimation("default", FEResources.getTexture("gui_selectArrow"), 8, 8, 6, 6, 0.1f);
	...
	public void render() {
		...
		if (CombatCalculator.shouldAttack(attacker, defender, attacker.getWeapon(), range)) {
			...
			if (mods.contains("Vegas")) {
				aHit = String.format("%d",
				        Math.max(0, Math.min(100, CombatCalculator.hitRate(attacker, defender))) / 2);
				aAtk = String.format("%d", CombatCalculator.calculatePreviewDamage(attacker, defender));
				aCrit = String.format("%d", Math.max(0, Math.min(100, attacker.crit() - defender.dodge())) * 2);
				if (attacker.getStats().spd >= defender.getStats().spd + 4)
					aMult *= 2;
				if (attacker.getWeapon().name.contains("Brave"))
					aMult *= 2;
				aEffective = attacker.getWeapon().effective.contains(defender.noGenderName());
			} else {// normal
				aHit = String.format("%d", Math.max(0, Math.min(100, CombatCalculator.hitRate(attacker, defender))));
				aAtk = String.format("%d", CombatCalculator.calculatePreviewDamage(attacker, defender));
				aCrit = String.format("%d", Math.max(0, Math.min(100, attacker.crit() - defender.dodge())));
				if (attacker.getStats().spd >= defender.getStats().spd + 4)
					aMult *= 2;
				if (attacker.getWeapon().name.contains("Brave"))
					aMult *= 2;
		...
		if (CombatCalculator.shouldAttack(defender, attacker, defender.getWeapon(), range)) {
			...
			if (mods.contains("Vegas")) {
				dHit = String.format("%d",
				        Math.max(0, Math.min(100, CombatCalculator.hitRate(defender, attacker))) / 2);
				dAtk = String.format("%d", CombatCalculator.calculatePreviewDamage(defender, attacker));
				dCrit = String.format("%d", Math.max(0, Math.min(100, defender.crit() - attacker.dodge())) * 2);
				...
				dHit = String.format("%d", Math.max(0, Math.min(100, CombatCalculator.hitRate(defender, attacker))));
				dAtk = String.format("%d", CombatCalculator.calculatePreviewDamage(defender, attacker) / 2);
				dCrit = String.format("%d", Math.max(0, Math.min(100, defender.crit() - attacker.dodge())));
				if (defender.getStats().spd >= attacker.getStats().spd + 4)
					dMult *= 2;
				if (defender.getWeapon().name.contains("Brave"))
					dMult *= 2;
		...
		// Borders
		Renderer.drawRectangle(x - 1, y - 1, x + 91, y + 132, renderDepth, FightStage.BORDER_DARK);
		Renderer.drawRectangle(x, y, x + 90, y + 35, renderDepth, FightStage.BORDER_LIGHT);
		Renderer.drawRectangle(x + 59, y + 35, x + 90, y + 96, renderDepth, FightStage.BORDER_LIGHT);
		Renderer.drawRectangle(x, y + 35, x + 31, y + 96, renderDepth, FightStage.BORDER_LIGHT);
		Renderer.drawRectangle(x, y + 96, x + 90, y + 131, renderDepth, FightStage.BORDER_LIGHT);

		// Top item and name
		Renderer.drawRectangle(x + 1, y + 1, x + 89, y + 34, renderDepth, attacker.getPartyColor());
		Renderer.drawString("default_med", attacker.name, x + 3, y + 3, renderDepth);
		new ItemDisplay((int) x + 7, (int) y + 16, attacker.getWeapon(), false).render(null, aEffective, timer);
		...
		Renderer.drawRectangle(x + 60, y + 34, x + 89, y + 96, renderDepth, attacker.getPartyColor());
		...
		// Stats text
		Renderer.drawRectangle(x + 31, y + 35, x + 59, y + 96, renderDepth, FightStage.NEUTRAL);
		...
		// Defender stats
		Renderer.drawRectangle(x + 1, y + 35, x + 30, y + 97, renderDepth, defender.getPartyColor());
		...
		Renderer.drawRectangle(x + 1, y + 97, x + 89, y + 130, renderDepth, defender.getPartyColor());
		Renderer.drawString("default_med", defender.name, x + 3, y + 99, renderDepth);
		new ItemDisplay((int) x + 7, (int) y + 112, defender.getWeapon(), false).render(null, dEffective, timer);
