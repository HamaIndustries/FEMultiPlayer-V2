...
public class TeamBuilderStage extends Stage {
	...
	public void render() {
		...
		List<String> stats = Arrays.asList("Lvl", "HP", "Str", "Mag", "Skl", "Spd", "Lck", "Def", "Res", "Mov");

		Renderer.drawBorderedRectangle(9, table_ystart - 2, 471, table_ystart + 14, 0.9f, FightStage.NEUTRAL,
		        FightStage.BORDER_LIGHT, FightStage.BORDER_DARK);

		Renderer.drawString("default_med", "Name", name, table_ystart, 0.5f);
		Renderer.drawString("default_med", "Class", classPos, table_ystart, 0.5f);
		int x = lv;
		for (String s : stats) {
		...
		Renderer.drawBorderedRectangle(9, yStart - 6, 471, yStart + vgap * units.size() - 2, 0.9f, FightStage.NEUTRAL,
		        FightStage.BORDER_LIGHT, FightStage.BORDER_DARK);

		int y = yStart;
		for (Unit u : units) {
			Renderer.drawString("default_med", u.name, name, y, 0.5f);
			Renderer.drawString("default_med", u.getUnitClass().name, classPos, y, 0.5f);
			x = lv;
			for (String s : stats) {
			...
			for (KeyboardEvent ke : keys) {
				if (ke.state) {
					if (ke.key == FEResources.getKeyMapped(Keyboard.KEY_Z)) {
						...
						if (cursor.on) {
							FEMultiplayer
							        .setCurrentStage(new UnitBuilderStage(units.get(cursor.getIndex()), this, session));
						} else {
							buttons.get(currButton).setHover(false);
