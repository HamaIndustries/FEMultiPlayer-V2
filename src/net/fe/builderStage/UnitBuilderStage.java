...
public class UnitBuilderStage extends Stage {
	...
	public UnitBuilderStage(Unit u, TeamBuilderStage s, Session session) {
		...
		shop = new ShopMenu(SHOP_X, SHOP_Y,
		        (session != null ? session.getModifiers() : java.util.Collections.emptySet()));
		shop.clearSelection();
	...
	private class Normal extends State {
		 ...
		 */
		public void render() {
			Renderer.drawBorderedRectangle(INFO_X, INFO_Y, INFO_X + INFO_W, INFO_Y + INFO_H, 1, FightStage.NEUTRAL,
			...
			if (inv.getSelectedIndex() < inv.size() && inv.getSelection() != null) {
				...
				Renderer.drawString("default_med", "Skill: " + unit.getTriggers().get(0).getClass().getSimpleName(),
