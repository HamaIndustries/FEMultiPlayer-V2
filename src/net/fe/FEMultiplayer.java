...
public class FEMultiplayer extends Game {
	...
	public void init(int width, int height, String name) {
		...
		ByteBuffer icon32;
		try {
			icon16 = ByteBuffer.wrap(TextureLoader
			        .getTexture("PNG", ResourceLoader.getResourceAsStream("res/gui/icon16.png")).getTextureData());
			icon32 = ByteBuffer.wrap(TextureLoader
			        .getTexture("PNG", ResourceLoader.getResourceAsStream("res/gui/icon32.png")).getTextureData());
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void testDraftStage() {
		...
		testSession = new Session(new net.fe.overworldStage.objective.Rout(), "test", 6, new java.util.HashSet<>(),
		        new net.fe.game.pick.Draft());
	 ...
	 * Test fight stage.
	 * 
	 * Use debugStat on units to mess with fights, is also useful for testing
	 * animations. Intentionally crashes at the end for lack of session info, so
	 * don't worry about it.
	 * 
	 * range can be set by messing with map.addUnit(), and you can change all
	 * the other unit-related things to test them out. I can see us using this
	 * to use this bit for speed testing in the future, if I ever make a gui or
	 * something for it.
	 * 
	...
	public void testFightStage() {
		...
		CombatCalculator calc = new CombatCalculator(new UnitIdentifier(u1), new UnitIdentifier(u2),
		        FEMultiplayer::getUnit);
	...
	/**
	 * Send. Used by the game to tell the unit on either client to attempt an
	 * action.
	 *
	...
	/**
	 * Disconnect from game. Allows for resetting server and client if
	 * triggered, but is not used in all situations.
	 *
	 * @param message the message
	 */
	public static void disconnectGame(String message) {
		/*
		 * //wouldn't be hard to use something like this to reset to lobby
		 * rather than quit the game: //at the moment this disconnect is only in
		 * a few places between stages, i.e. while waiting //so it's not too bad
		 * to quit the game. Player leaver = null; for(Player p :
		 * session.getPlayers()) { if(p.getID() == message.origin) { leaver = p;
