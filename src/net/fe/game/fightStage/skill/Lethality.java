...
public class Lethality extends CombatTrigger {
	 ...
	 * 
	 * Info: Sets the user's Hit/Str to 9000, guaranteeing a kill (except on
	 * FalseProof). Will not proc on Lords.
	 * 
	 * Chance: Skl/3
	 */
	public Lethality() {
	...
	public boolean attempt(Unit user, int range, Unit opponent) {
		return (!opponent.getUnitClass().name.equals("Lord")) && RNG.get() < user.getStats().skl / 3;
	}
