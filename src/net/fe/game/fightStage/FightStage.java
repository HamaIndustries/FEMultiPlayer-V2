...
import net.fe.game.fightStage.anim.BackgroundEffect;
import net.fe.game.fightStage.anim.DodgeAnimation;
import net.fe.game.fightStage.anim.HUD;
import net.fe.game.fightStage.anim.HitEffect;
import net.fe.game.fightStage.anim.MagicAttack;
import net.fe.game.fightStage.anim.MagicEffect;
import net.fe.game.fightStage.anim.MissEffect;
import net.fe.game.fightStage.anim.NoDamageEffect;
import net.fe.game.fightStage.anim.Platform;
import net.fe.game.fightStage.anim.SkillIndicator;
import net.fe.game.unit.BattleStats;
...
public class FightStage extends Stage {
	 ...
	 * @param returnTo the stage to return to after this stage has played its
	 *        animation
	 * @param returnCallback a callback to be run after this stage has played
	 *        its animation
	 */
	public FightStage(UnitIdentifier u1, UnitIdentifier u2, ArrayList<AttackRecord> attackQ,
	        ClientOverworldStage returnTo, Runnable returnCallback) {
		super(u1.partyColor.equals(u2.partyColor) ? "curing"
		        : u1.partyColor.equals(FEMultiplayer.getLocalPlayer().getParty().getColor()) ? "fight" : "defense");
		shakeTimer = 0;
		prevShakeTimer = 0;
		timer = 0;
		done = false;
	...
	public void beginStep(List<Message> messages) {
		...
		if (attackQ.size() != 0) {
			...
			System.out.println(left.name + " HP:" + left.getHp() + " | " + right.name + " HP:" + right.getHp());
	...
	private void processAttackQueue() {
		...
		if (currentEvent == START) {
			...
			if (rec.damage == 0) {
				...
				if (!defender.getPartyColor().equals(attacker.getPartyColor())) {
					if (rec.damage > 0) {
						...
						attacker.addBattleStats(new BattleStats(/* kills = */ 0,
						        /* assists = */ 0, /* damage = */ rec.damage,
						        /* healing = */ rec.drain));
					}
				} else {
					attacker.addBattleStats(new BattleStats(/* kills = */ 0,
					        /* assists = */ 0, /* damage = */ 0, /* healing = */ -rec.damage));
				}
				if (rec.damage > 0) {
			...
			if (dhp.getHp() == 0) {
				...
				currentEvent = DYING;////////// TODO: check here for bug,
				                     ////////// DYING->DONE illegal
				                     ////////// transition
			} else {
	 ...
	 */
	public static ArrayList<String> analyzeAnimation(String animName, String suffix, boolean stripNums) {
	...
	public boolean hasHitEffects() {
		for (Entity e : entities) {
			if (e instanceof HitEffect || e instanceof MissEffect || e instanceof NoDamageEffect) {
				...
				// System.out.println("still have hiteffects
