...
public class BackgroundEffect extends Entity {
	...
	public BackgroundEffect(String name, boolean left) {
		...
		sprite.addAnimation("default",
		        new Animation(FightStage.getPreload(name), data.frameWidth, data.frameHeight, data.frames, data.columns,
		                data.offsetX, data.offsetY, data.speed == 0.0f ? 0.05f : data.speed, data.getBlendMode()) {
	...
	public void render() {
		...
		if (sprite.getCurrentAnimation().getHeight() == 1)
			t.setScale(240, 160);
		sprite.render(FightStage.CENTRAL_AXIS - 120, FightStage.FLOOR - 104, 0, t);
	}
}
