...
public final class Weapon extends Item {
	 ...
	 * @param range the weapon's range. If this weapon is expected to travel
	 *        over a network, this should be serializable and have a stable
	 *        hashCode. Note that lambdas don't generally satisfy this property
	 */
	public Weapon(String name, int maxUses, int id, int cost, Type type, int mt, int hit, int crit,
	        Function<Statistics, List<Integer>> range, Statistics modifiers, List<String> effective, String pref) {
	...
	public Weapon getCopy() {
		return new Weapon(name, getMaxUses(), id, getCost(), type, mt, hit, crit, range, modifiers, effective, pref);
	}

	/**
	 * Returns an item identical to this one, with the exception of an updated
	 * mt, hit and crit
	 */
	public Weapon getCopyWithNewMtHitCrit(int newmt, int newhit, int newcrit) {
		return new Weapon(name, getMaxUses(), id, getCost(), type, newmt, newhit, newcrit, range, modifiers, effective,
		        pref);
	}
	...
	@Override
	public int hashCode() {
		return (((((super.hashCode() * 31 + this.type.ordinal()) * 31 + this.range.hashCode()) * 31
		        + this.modifiers.hashCode()) * 31 + java.util.Objects.hashCode(this.pref)) * 31
		        + this.effective.hashCode()) * 31 + (crit << 14 + hit << 7 + mt);
	}
	...
	public boolean equals(Object other) {
		if (other != null && other instanceof Weapon) {
			...
			if (o2.canEquals(this)) {
				return super.equals(o2) && this.mt == o2.mt && this.hit == o2.hit && this.crit == o2.crit
				        && this.type == o2.type && this.range.equals(o2.range) && this.effective.equals(o2.effective)
				        && (this.pref == null ? o2.pref == null : this.pref.equals(o2.pref))
	...
	public String toString() {
		return "Weapon [" + "name: " + name + "; " + "maxUses: " + getMaxUses() + "; " + "uses: " + getUses() + "; "
		        + "id: " + id + "; " + "cost: " + getCost() + "; " + "type: " + type + "; " + "mt: " + mt + "; "
		        + "hit: " + hit + "; " + "range: " + range + "; " + "modifiers: " + modifiers + "; " + "effective: "
		        + effective + "; " + "pref: " + pref + "]";
	}
}
