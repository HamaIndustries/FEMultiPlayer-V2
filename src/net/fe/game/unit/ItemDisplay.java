...
public class ItemDisplay extends Entity {
	...
	public void render(Transform t, boolean effective, float timer) {
		...
		Renderer.render(weaponIcon, col / 8.0f, row / 10.0f, (col + 1) / 8.0f, (row + 1) / 10.0f, x - 1, y, x + 16,
		        y + 17, renderDepth, t, args, chu.engine.anim.BlendModeArgs.ALPHA_BLEND);
		FEResources.getBitmapFont("default_med").render(item.name, x + 16, y + 3, renderDepth, t);
		if (equip) {
			Renderer.render(e, 0, 0, 1, 1, x + 10, y + 10, x + 16, y + 17, renderDepth, t);
		}
	}
