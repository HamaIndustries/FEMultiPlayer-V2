...
/**
 * Abstract class that determines how an object deals with collisions. Entities
 * without defined hitboxes cannot collide with other objects. Entities with
 * defined hitboxes will consult their hitbox for collision detection. Child
 * classes of Hitbox define how the entity checks for collision.
 * 
 * @author Shawn
 *
 */
...
public class Hitbox {
	...
	private static int checkCollision(RectangleHitbox a, RectangleHitbox b, int offsetX, int offsetY) {
		if ((int) (a.getX() + offsetX) >= (int) (b.getX() + b.getWidth()))
			return -1;
		if ((int) (a.getX() + offsetX + a.getWidth()) <= (int) (b.getX()))
			return -1;
		if ((int) (a.getY() + offsetY) >= (int) (b.getY() + b.getHeight()))
			return -1;
		if ((int) (a.getY() + offsetY + a.getHeight()) <= (int) (b.getY()))
			return -1;
